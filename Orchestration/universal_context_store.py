...
class UniversalContextStore:
    ...
    def __init__(self, redis_client: redis.Redis):
        ...
        self.context_extractors = {
            ...
            ContextType.ICP_GENERATION: self._extract_icp_generation_context,
        }
    ...
    def _map_agent_to_context_type(self, agent_id: str) -> Optional[ContextType]:
        ...
        mapping = {
            ...
            "icp_generator_agent": ContextType.ICP_GENERATION,
        }
    ...
    def _extract_lead_mining_context(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Extract lead mining context from nested results structure."""
        
        # Handle nested structure from semantic orchestrator
        lead_data = None
        
        # Check if results are nested under 'results.lead_mining_agent'
        if "results" in results and isinstance(results["results"], dict):
            if "lead_mining_agent" in results["results"]:
                lead_data = results["results"]["lead_mining_agent"]
        
        # Check direct structure
        elif "qualified_leads" in results:
            lead_data = results
        
        # Check if results itself is the lead mining data
        elif "mining_success" in results:
            lead_data = results
        
        if not lead_data:
            self.logger.debug(f"No lead mining data found in results structure: {list(results.keys())}")
            return None
        ...
        leads_found = lead_data.get("leads_found", 0)
        mining_success = lead_data.get("mining_success", False)
        
        if not leads_data and not leads_found:
            self.logger.debug(f"No extractable lead data found")
            return None
        
        extracted = {
            "qualified_leads": leads_data,
            "leads_found": leads_found,
            "icp_criteria_used": lead_data.get("icp_criteria_used", {}),
            "apollo_status": "connected" if mining_success else "failed",
            ...
            "csv_file": lead_data.get("leads_csv_file"),
        }
        
        self.logger.info(f"✅ Extracted lead mining context: {len(leads_data)} leads")
        return extracted
    ...
    def _extract_icp_generation_context(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Extract ICP generation context."""
        
        # Handle nested structure
        icp_data = None
        if "results" in results and "icp_generator_agent" in results["results"]:
            icp_data = results["results"]["icp_generator_agent"]
        elif "icp_generated" in results:
            icp_data = results
        
        if not icp_data:
            self.logger.debug(f"No ICP generation data found in results structure: {list(results.keys())}")
            return None
        
        extracted = {
            "icp_criteria": icp_data.get("icp_criteria", {}),
            "customer_patterns": icp_data.get("customer_patterns", {}),
            "source_customers_count": icp_data.get("source_customers_count", 0),
            "icp_confidence": icp_data.get("icp_confidence", 0),
            "recommendations": icp_data.get("icp_result", {}).get("recommendations", []),
            "ready_for_lead_mining": icp_data.get("icp_generated", False)
        }
        
        self.logger.info(f"✅ Extracted ICP generation context with {extracted['icp_confidence']:.2f} confidence")
        return extracted
