...
class ContextAwareResponseAnalyzer:
    ...
    async def analyze_response_needed(
        ...
        try:
            ...
            # FIXED: Handle both AIResponse objects and direct strings for compatibility
            ...
            return self._parse_response_decision(response_text, message)
...
2. AGENT_EXECUTION: User wants to create, build, generate something new OR requests business actions
   - Examples: "Create a website", "Design a logo", "Research the market for electric cars", "Generate leads", "Find prospects", "Monitor my emails", "Set up email sequences", "Generate ICP", "Create customer profile"
   - Use when: Clear action request for new deliverables OR business service requests
   - Key indicators: "create", "build", "design", "generate", "make", "develop", "find", "get", "mine", "monitor", "set up", "analyze", "research", "icp", "customer profile"
...
- CRITICAL: "generate icp", "create icp", "icp from", "customer profile", "ideal customer", "grab icp", "extract icp" = AGENT_EXECUTION (not advice)
- CRITICAL: ANY mention of "ICP" or "ideal customer profile" = AGENT_EXECUTION (not advice)
- CRITICAL: ANY request about finding, generating, or getting leads/prospects/customers = AGENT_EXECUTION
- CRITICAL: ANY request about generating, creating, or analyzing ICPs/customer profiles = AGENT_EXECUTION
- If user requests business actions like "monitor", "set up", "analyze", "research", use AGENT_EXECUTION
- SPECIAL CASE: If user asks about "capabilities" of specific systems (CRM, email, etc.), use AGENT_EXECUTION to get real-time status
- If user asks "what CRM capabilities", "what email capabilities", etc., use AGENT_EXECUTION (not direct_answer)
