...
from base_engine import AIEngineConfig
...
class CapabilityCategory(str, Enum):
    ...
    CONTENT_MARKETING = "content_marketing"  # NEW: Specific marketing capability
...
class CapabilityAgentRegistry:
    ...
    def _initialize_registry(self):
        ...
        # LinkedIn automation capabilities (separate from general social monitoring)
        self.register_capability(AgentCapability(
            agent_id="linkedin_scraping_agent",
            capability_category=CapabilityCategory.LINKEDIN_AUTOMATION,
            specific_skills=["viral_posts_scraping", "linkedin_trends", "prospect_identification", "comment_analysis", "job_monitoring", "linkedin_engagement_tracking"],
            execution_requirements={"linkedin_credentials": True, "selenium": True, "anti_detection": True, "browser_automation": True}
        ))
        
        # Content marketing capabilities - FIXED: Use dedicated CONTENT_MARKETING capability
        self.register_capability(AgentCapability(
            agent_id="content_marketing_agent",
            capability_category=CapabilityCategory.CONTENT_MARKETING,
            specific_skills=["content_gap_analysis", "seo_optimization", "content_calendar", "content_distribution", "content_strategy", "marketing_campaigns", "blog_content", "social_media_strategy"],
            execution_requirements={"cms_apis": True, "seo_tools": True, "ai_analysis": True}
        ))
        
        # Communication Monitoring capabilities (currently integrated into email orchestration)
        # Note: Standalone communication monitoring agent is not yet implemented
        # Gmail monitoring is handled by the advanced_email_orchestration_agent
        ...
        self.register_capability(AgentCapability(
            ...
            specific_skills=[
                ...
                # Gmail connection and monitoring capabilities
                "gmail_connection_check", "gmail_status_monitoring", "email_account_verification",
        ...
        # CRM Operations capabilities
        self.register_capability(AgentCapability(
            agent_id="crm_operations_agent",
            capability_category=CapabilityCategory.CRM_OPERATIONS,
            specific_skills=[
                "bidirectional_crm_sync", "hubspot_integration", "salesforce_integration", "pipedrive_integration",
                "ai_deduplication", "conflict_resolution", "field_mapping", "data_transformation",
                "cross_agent_collaboration", "contact_intelligence", "human_approval_workflows",
                "sync_monitoring", "analytics_reporting", "data_quality_analysis"
            ],
            execution_requirements={
                "crm_apis": True, "claude_ai": True, "data_processing": True,
                "sync_engine": True, "analytics_tools": True
            },
            estimated_duration=180
        ))
        
        # ICP Generation capabilities (separate from lead generation)
        self.register_capability(AgentCapability(
            agent_id="icp_generator_agent",
            capability_category=CapabilityCategory.ICP_GENERATION,
            specific_skills=["customer_analysis", "pattern_recognition", "icp_generation", "hubspot_integration", "ideal_customer_profile"],
            execution_requirements={"ai_analysis": True, "customer_data": True}
        ))
...
class SemanticRequestParser:
    ...
    def _build_semantic_analysis_prompt(self, user_request: str, context: Optional[Dict[str, Any]]) -> str:
        ...
        capability_descriptions = {
            ...
            CapabilityCategory.LEAD_GENERATION: "Finding and mining new leads, prospect discovery, Apollo integration for lead generation",
            CapabilityCategory.CONTENT_CREATION: "Writing basic copy, blog posts, social content",
            CapabilityCategory.CONTENT_MARKETING: "Comprehensive marketing campaigns, content strategy, SEO optimization, content calendars, marketing automation, and content distribution across channels",
            ...
            CapabilityCategory.LINKEDIN_AUTOMATION: "LinkedIn scraping, viral post analysis, prospect identification, and engagement tracking",
            ...
            CapabilityCategory.CRM_OPERATIONS: "Bi-directional CRM sync (HubSpot, Salesforce, Pipedrive), AI-powered deduplication, field mapping, conflict resolution, and cross-agent collaboration",
            CapabilityCategory.ICP_GENERATION: "Creating Ideal Customer Profile (ICP) based on existing customer analysis, not for finding new leads"
        }
...
1. "generate ICP", "ideal customer profile", "analyze my customers" → ICP_GENERATION (icp_generator_agent)
2. "find leads", "generate leads", "mine leads", "prospect search" → LEAD_GENERATION (lead_mining_agent)
3. ICP is for analyzing EXISTING customers, Lead Generation is for finding NEW customers
...
{{
    ...
    "business_context": {{
        ...
        "company_size": "extracted size if mentioned", 
        "target_market": "extracted target market if mentioned",
        "business_model": "extracted model if mentioned",
        "relevant_context": "any other relevant business context"
    }},
    "user_preferences": {{
...
1. Lead generation: "generate leads", "find prospects", "find customers", "mine leads" → lead_mining_agent with LEAD_GENERATION
2. Marketing campaigns: "marketing campaign", "content strategy", "content calendar", "SEO", "marketing automation" → content_marketing_agent with CONTENT_MARKETING
3. Basic content: "write copy", "blog post", "social media post" → content_creator_agent with CONTENT_CREATION
4. Sales materials: "sales email", "pitch deck", "sales copy" → sales_outreach_agent with SALES_OUTREACH
5. LinkedIn automation: "linkedin", "viral posts", "linkedin trends", "linkedin content" → linkedin_scraping_agent with LINKEDIN_AUTOMATION

LINKEDIN AGENT PRIORITY: If request mentions "linkedin", "viral posts", "linkedin trends", "linkedin content", "linkedin scraping", or "linkedin engagement", ALWAYS use linkedin_scraping_agent with LINKEDIN_AUTOMATION capability.

MARKETING AGENT PRIORITY: If request mentions "marketing strategy", "content marketing", "marketing campaigns", "content calendar", "SEO optimization", "content distribution", or "marketing automation", ALWAYS use content_marketing_agent with CONTENT_MARKETING capability.
...
"""
        ...
        for cap in capabilities_list:
            ...
            try:
                ...
                capability_mapping = {
                    ...
                    'marketing': CapabilityCategory.CONTENT_MARKETING,
                    'marketing campaign': CapabilityCategory.CONTENT_MARKETING,
                    'marketing campaigns': CapabilityCategory.CONTENT_MARKETING,
                    'content marketing': CapabilityCategory.CONTENT_MARKETING,
                    'content strategy': CapabilityCategory.CONTENT_MARKETING,
                    'content calendar': CapabilityCategory.CONTENT_MARKETING,
                    'marketing strategy': CapabilityCategory.CONTENT_MARKETING,
                    'marketing automation': CapabilityCategory.CONTENT_MARKETING,
                    'seo': CapabilityCategory.CONTENT_MARKETING,
                    'seo optimization': CapabilityCategory.CONTENT_MARKETING,
                    'content distribution': CapabilityCategory.CONTENT_MARKETING,
                    'marketing materials': CapabilityCategory.CONTENT_MARKETING,
                    ...
                    'linkedin': CapabilityCategory.LINKEDIN_AUTOMATION,
                    'linkedin scraping': CapabilityCategory.LINKEDIN_AUTOMATION,
                    'viral posts': CapabilityCategory.LINKEDIN_AUTOMATION,
                    'linkedin trends': CapabilityCategory.LINKEDIN_AUTOMATION,
                    'linkedin content': CapabilityCategory.LINKEDIN_AUTOMATION
                }
    ...
    def _parse_ai_response(self, response: str, original_request: str) -> SemanticUnderstanding:
        ...
        try:
            ...
            # FIXED: Ensure business_context is always a dict to prevent AttributeError
            business_context_raw = data.get("business_context", {})
            if isinstance(business_context_raw, str):
                # If AI returned business_context as string, convert to dict
                business_context = {"description": business_context_raw}
                logger.warning(f"AI returned business_context as string, converted to dict: {business_context}")
            elif isinstance(business_context_raw, dict):
                business_context = business_context_raw
            else:
                business_context = {}
                logger.warning(f"AI returned invalid business_context type {type(business_context_raw)}, using empty dict")
            ...
            return SemanticUnderstanding(
                ...
                business_context=business_context,  # Use the validated dict
